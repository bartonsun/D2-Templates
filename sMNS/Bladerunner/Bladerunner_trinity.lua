...
local kef = 1
------------------------------------------------------------------------------------------
--- режим рынка (только режим 1x2)
--- 1. отключен
--- 2. включен
local market_mode = 1
------------------------------------------------------------------------------------------
--- режим наемников центр/трежа
...
function zoneMageZone4()
	local spells1 = chooseSpells(spells_t4_1, sp3)
	local spells2 = chooseSpells(spells_t4_2, sp3)
	for _,v in pairs(spells_special[1]) do
		table.insert(spells1, v)
	end
	for _,v in pairs(spells_special[2]) do
		table.insert(spells2, v)
	end
	table.insert(spells1, spells_wards[1])
	table.insert(spells2, spells_wards[2])
	return {
		{
			name = mage_c_data[1].name,
			description = mage_c_data[1].description,
			spells = spells1
		},
		{
			name = mage_c_data[2].name,
			description = mage_c_data[2].description,
			spells = spells2
		}
	}
end
...
function zoneMerchZone4()
	return {
		{
			name = merchant_c_data[1].name,
			description = merchant_c_data[1].description,
			goods = merchZone41()
		},
		{
			name = merchant_c_data[2].name,
			description = merchant_c_data[2].description,
			goods = merchZone42()
		}
	}
end
...
local valueRuinZone4 = { min = 900*kef, max = 900*kef*kr }
local goldRuinZone4 = { min = 400, max = 450 }
local ruins = {
		{
			name = ruins_c_data[2],
			gold = goldRuinZone4,
	...
	if gmode == 1 then
		table.insert(ruins, zoneRuinZone3()[1])
	end
	return ruins
end
...
function getMinesRespZ(race)
	local mines = { -- FFA/1x2: 3 = 1 золото, 1 родная мана, 1 "важная" мана
...
function getMinesRespZ2(race)
	local mines = gmm(
		...
		{	-- 1x2: 3 = 1 золото, 1 родная мана, 1 "неважная" мана
...
function getMinesZone2(race)
	local mines = gmm(
		...
		{	-- 1x2: 3 = 1 золото, 1 "важная" мана, 1 "неважная" мана
...
function getMinesZone3(race)
	local mines = {
		[Race.Human] = { deathMana = mines2[2], infernalMana = mines2[1]},
		[Race.Dwarf] = { deathMana = mines2[2], infernalMana = mines2[1]},
		[Race.Undead] = { lifeMana = mines2[2], runicMana = mines2[1]},
		[Race.Heretic] = { lifeMana = mines2[2], runicMana = mines2[1]},
		[Race.Elf] = { runicMana = mines2[2], deathMana = mines2[1]},
	}
	return mines[race]
...
-- Источники т4
function getMinesZone4()
	local gold_mines = gmm(2, 1)
	if treasure_mode == 1 then
		gold_mines = gold_mines + gmm(3, 2)
	end
	local mines = {
		gold = gold_mines,
		lifeMana = 1,
...
-- Наёмники в зоне 5
function zoneMercenZone5(merc_id)
	local mercen = {}
	local units = {}
	mercen['name'] = merc_c_data[merc_id].name
	mercen['description'] = merc_c_data[merc_id].description
	if merc_id == 1 then
		units = unitsMercen1()
	elseif merc_id == 2 then
		units = unitsMercen2()
	elseif merc_id == 3 then
		units = unitsMercen3()
	end
	if merc_c_data[merc_id].unit then
		table.insert(units, merc_c_data[merc_id].unit)
	end
	mercen['units'] = units
	return { mercen }
end
...
function mmc(what1, what2)
	if market_mode == 1 or gmode == 1 then
		return what1
	elseif market_mode == 2 and gmode == 2 then
		return what2
	end
end
	...
	return { -- 500хп*2 -- Эликсир -75 лечения / оз+25%
		...
		loot = {
			items = {
				...
				{ id = 'g001ig0378', min = 1, max = 1}, --хил75
				{ id = 'g000ig7002', min = 1, max = 1 }, --кольцо100
				rnd({ id = 'g001ig0026', min = 1, max = 1 },{ id = 'g001ig0130', min = 1, max = 1 },{ id = 'g001ig0561', min = 1, max = 1 }), -- Аура регенерации (+10 реген аое)/эликсир регенерации/эликсир самопожертвования
				{ id = b15(), min = 1, max = 1}, -- случ. банка15
...
function getZone5Stacks()
	...
	if gmode == 1 then
		table.insert(stacks, z3Stacks1('g001ig0146'))
		table.insert(stacks, z3Stacks1('g001ig0148'))
		table.insert(stacks, z3Stacks1('g001ig0149'))
		table.insert(stacks, z3Stacks1('g001ig0147'))
		table.insert(stacks, z3Stacks2('g000ig0003'))
		table.insert(stacks, z3Stacks2('g000ig0009'))
		...
		trainers = {
			{
				...
				description = trainer_c_data[tid].description,
			}
		},
		mines = getMinesZone3(),
		ruins = zoneRuinZone3(),
		stacks = {
			z3Stacks1(rms()), z3Stacks1(rms()),
			z3Stacks2(b30), z3Stacks2(b30),
		}
	}
end
...
function getTreasureZone4(zoneId, zoneSize)
	...
	if treasure_mode == 1 and mercenary_mode > 1 then
		...
		-- первый лагерь
		{
			name = merc_c_data[1].name,
			description = merc_c_data[1].description,
			units = merc_units[1]
		},
		-- второй лагерь
		{
			name = merc_c_data[2].name,
			description = merc_c_data[2].description,
			units = merc_units[2]
		},
	}
	end
	...
	return {
		...
		trainers = gmm(
			{
				{
					name = trainer_c_data[1].name,
					description = trainer_c_data[1].description
				}, {
					name = trainer_c_data[2].name,
					description = trainer_c_data[2].description
			}
		},
		""
		),
		resourceMarkets = rMarketsZone4(),
		stacks = getZone5Stacks(),
		bags = bags3(),
	}
end
...
function getDescription()
	return gmm(
		'Черная зона в центре, ее должны касаться: синяя, белая, желтая, коричневая, т.синяя, т.серая',
		'Черная зона в центре, ее должны касаться: серая, т.серая, коричневая, т.синяя.\nСиняя должна касяться т.серой и т.синей\nСерая должна касаться белой и желтой'
	)
end
...
function getTemplateContents(races, scenarioSize, parameters)
	...
	if parameters then
		if parameters[1] then
			...
			smode = math.ceil(parameters[1] / 2)
			if n == 1 then
				gmode = 1
			elseif n == 0 then
				gmode = 2
		...
		if parameters[3] then
			mercenary_mode = parameters[3]
		end
		if parameters[4] then
			treasure_mode = parameters[4]
		end
